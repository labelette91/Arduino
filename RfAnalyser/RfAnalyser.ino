 ...
 // send data only when you receive data:
    if (Serial.available() > 0) {
            // read the incoming byte:
             Serial.read();
            if (incomingByte==0)incomingByte=1;else incomingByte=0;
            Serial.println();
    }
    ...
    if (p != 0) {
      if (incomingByte){
        ...
        Pulse[PWr++]=p;
	if ( (p >= 300)&& (p < 700)){
          Dt = (millis() - LastReceive);
          if (Dt>100){  Serial.println(); }
          //trig record
          if (FgRecord==0){
            //print last 2 bytes
//            Serial.print("..."); 
            print(Pulse[PWr-3]);  print(Pulse[PWr-2]);
          }
          FgRecord=1;
          LastReceive = millis() ;
        }
	if ( (p <= 300)){ FgRecord=0;  }
        if (FgRecord==1){ print(p); }
