...
//    Serial.begin(115200);
   ...
   // initialize the LED pin as an output:
    ...
    if ( (gid[0]!=0)&&(gid[1]!=0)&&(gid[2]!=0)&&(gid[3]!=0) )
...
/*     Dt = (millis() - LastReceive)/1000;
    ...
    if (learningInProgress==1){
        ...
        if ( (pinlearning==0) &&  ((millis()-LearningTime) >3000 )  ){
            gid[0]        =  0 ;            /* id emetteur 0..3  */
            gid[1]        =  0 ;            /* id emetteur 0..FF */
            gid[2]        =  0 ;            /* id emetteur 0..FF */
            gid[3]        =  0;             /* id emetteur 0..FF */
            learningInProgress=0;
            for (byte i=0;i<4;i++)
                EEPROM.put(i,gid[i]);
            digitalWrite(PINSTATUS,HIGH);
